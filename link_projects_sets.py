...
def main():
    ...
    RE_DUPPFX = [(re.compile(r'\b(Fotovoltaica|Fotovoltaico)\s+(?:FV|PFV|PSFV)\b', re.I), r'\1'),
                 (re.compile(r'\b(PSFV|PFV|FV)\s+(?:PFV|FV|PSFV)\b'), r'\1')]
    for p in pj['proyectos']:
        nm = p.get('nombre') or ''
        for rx, rp in RE_DUPPFX:
            nm = rx.sub(rp, nm)
        p['nombre'] = ' '.join(nm.split())
        for f in backfill(p):
            stats[f] += 1
