...
def main():
    ...
    names = list(idx.keys())
    # indice adicional: claves del historico (CSVs DSO + Mapas) no presentes en el Monitor actual
    try:
        import os as _os
        hist_path = _os.path.join(_os.path.dirname(_os.path.abspath(pj_path)) or '.', 'sets_history.json')
        hist = json.load(open(hist_path, encoding='utf-8')); hist.pop('_v', None)
        caps_hist = {k: None for k in hist if k not in caps}
        idx_hist = build_monitor_index(caps_hist)
        for n_, lst in idx_hist.items():
            if n_ not in idx:
                idx[n_] = lst
        names = list(idx.keys())
        print(f'indice SETs: {len(caps)} Monitor + {len(caps_hist)} solo-historico')
    except Exception as e:
        print('WARN historico:', e)
    ...
    by_first = {}
    for n_ in names:
        by_first.setdefault(n_[:3], []).append(n_)
    ...
    def link_set_fast(p):
        ...
        k, h = fast_link(nombre, kvs)
        if k: return k, h
        # tokens del monitor contenidos en la subestacion (kv obligatoria)
        toks = set(nombre.split())
        if kvs:
            tc = []
            t0 = nombre.split()[0] if nombre.split() else ''
            for n_ in tok1.get(t0, []):
                ntoks = set(n_.split())
                if ntoks and ntoks <= toks:
                    for key, kv in idx[n_]:
                        if kv in kvs: tc.append((key, kv, len(n_)))
            if tc:
                tc.sort(key=lambda c: (-c[2], -c[1]))
                return tc[0][0], 'prefijo'
        # prefijo sin exigir tension (nombres truncados del Monitor, ej. AGUADULC)
        t0 = nombre.split()[0] if nombre.split() else ''
        best_p = None
        for n_ in tok1.get(t0, []):
            if len(n_) >= 5 and (nombre.startswith(n_) or n_.startswith(nombre)):
                if best_p is None or len(n_) > len(best_p): best_p = n_
        if best_p:
            cands = idx[best_p]
            wk = [c for c in cands if c[1] in kvs]
            return max(wk or cands, key=lambda c: c[1])[0], 'prefijo'
        # fuzzy restringido al bucket de las 3 primeras letras
        if kvs:
            from difflib import SequenceMatcher as _SM
            best, best_r = None, 0.0
            for n_ in by_first.get(nombre[:3], []):
                if abs(len(n_) - len(nombre)) > 6: continue
                rr = _SM(None, nombre, n_).ratio()
                if rr > best_r: best, best_r = n_, rr
            if best and best_r >= 0.94:
                wk = [c for c in idx[best] if c[1] in kvs]
                if wk: return max(wk, key=lambda c: c[1])[0], 'fuzzy'
        return None, None
    ...
    keep = [p for p in pj['proyectos'] if es_relevante(p)]
    lineas = [p for p in pj['proyectos'] if not es_relevante(p)]

    # Asociar lineas de evacuacion a proyectos identificados:
    # (a) expediente compartido; (b) nombre del proyecto contenido en el de la linea
    exp_idx = {}
    for p in keep:
        for e in (p.get('expedientes') or []):
            if e and len(str(e)) >= 6:
                exp_idx.setdefault(str(e).upper(), p)
    def core_name(p):
        nm = deaccent((p.get('nombre') or '').upper())
        nm = re.sub(r'^(?:PLANTA|PARQUE|INSTALACION)S?\s+(?:SOLAR\s+)?(?:FOTOVOLTAICA|FOTOVOLTAICO|EOLIC[OA]|FV|HIBRIDA)?\s*', '', nm)
    ...
    asociadas, descartadas = 0, 0
    for ln in lineas:
        parent = None
        for e in (ln.get('expedientes') or []):
            parent = exp_idx.get(str(e).upper())
            if parent: break
        if not parent:
            lnm = ' ' + re.sub(r'[^A-Z0-9 ]', ' ', deaccent((ln.get('nombre') or '').upper())) + ' '
            lnm = ' '.join(lnm.split())
            lnm = ' ' + lnm + ' '
            hits = [p for c, p in name_idx if c in lnm]
            if len({id(h) for h in hits}) == 1:
                parent = hits[0]
        if parent:
            dest = parent.setdefault('publicaciones_infra', [])
            vistos = {pub.get('id_boe') for pub in dest}
            for pub in (ln.get('publicaciones') or []):
                if pub.get('id_boe') in vistos: continue
                pub2 = dict(pub)
                pub2['infraestructura'] = ln.get('nombre')
                dest.append(pub2)
            parent.setdefault('infra_asociada', [])
            if ln.get('nombre') not in parent['infra_asociada']:
                parent['infra_asociada'].append(ln.get('nombre'))
            asociadas += 1
        else:
            descartadas += 1
    ...
    pj['proyectos'] = keep
    pj['total'] = len(keep)
    print(f"filtro lineas: {antes} -> {len(keep)} | asociadas a proyecto: {asociadas} | descartadas: {descartadas}")

    stats = {'potencia': 0, 'promotor': 0, 'subestacion': 0,
             'exacto': 0, 'tension': 0, 'prefijo': 0, 'fuzzy': 0, 'sin_match': 0}
    # ── Estandarizacion de nombres por tecnologia ──────────────────────
    # PSFV <nombre> (fotovoltaica) · PE <nombre> (eolica) · BESS <nombre> ...
    PFX_TEC = {'Fotovoltaica': 'PSFV', 'FV+BESS': 'PSFV', 'Eólica': 'PE', 'Eolica': 'PE',
               'Eólica+BESS': 'PE', 'BESS': 'BESS', 'Hidráulica': 'CH', 'Termosolar': 'TS',
               'Biometano': 'PB', 'Biomasa': 'PB'}
    RE_STRIP_PFX = re.compile(
        r'^(?:PSFV|PFV|PSF|ISFV|ISF|FV|P\.?E\.?|PE|BESS|CH|'
        r'(?:PLANTA|PARQUE|INSTALACION|INSTALACIÓN)(?:ES)?\s*(?:SOLAR(?:ES)?)?\s*'
        r'(?:FOTOVOLTAICA|FOTOVOLTAICO|EOLICA|EOLICO|EÓLICA|EÓLICO|FV)?S?)\s+', re.I)
    RE_NO_RENOMBRAR = re.compile(
        r'^(?:INSTALACIONES\s+COMUNES|INFRAESTRUCTURA|L[IÍ]NEA|LAT|LAAT|SUBESTACI[OÓ]N|SET\b|'
        r'AMPLIACI[OÓ]N|HIBRIDACI[OÓ]N|M[OÓ]DULO|REPOTENCIACI[OÓ]N|CENTRO|NUEVA)', re.I)
    for p in pj['proyectos']:
        nm = ' '.join((p.get('nombre') or '').split())
        pfx = PFX_TEC.get(p.get('tecnologia') or '')
        if pfx and nm and not RE_NO_RENOMBRAR.match(nm):
            resto = nm
            for _ in range(4):
                r2 = RE_STRIP_PFX.sub('', resto, count=1)
                if r2 == resto: break
                resto = r2
            resto = resto.strip(' -')
            resto = __import__('re').sub(r'^(?:de|del)\s+', '', resto, flags=2)
            if len(resto) >= 3:
                nm = pfx + ' ' + resto
        p['nombre'] = nm
        for f in backfill(p):
            stats[f] += 1
        key, how = link_set_fast(p)
    ...
    # ── Herencia local: proyectos hermanos ──────────────────────────────
    # (a) mismo promotor + mismo municipio  (b) misma subestacion privativa declarada
    prom_mun, priv_idx = {}, {}
    for p in pj['proyectos']:
        if not p.get('set_vinculada'): continue
        if p.get('promotor') and p.get('municipio'):
            k = (norm_name(p['promotor'])[:30], norm_name(p['municipio'])[:25])
            prom_mun.setdefault(k, (p['set_vinculada'], p['nombre']))
        if p.get('subestacion'):
            sk = norm_name(p['subestacion'])
            if len(sk) >= 6:
                priv_idx.setdefault(sk, (p['set_vinculada'], p['nombre']))
    heredados = 0
    for p in pj['proyectos']:
        if p.get('set_vinculada'): continue
        hit = None
        if p.get('subestacion'):
            sk = norm_name(p['subestacion'])
            if len(sk) >= 6:
                hit = priv_idx.get(sk)
        if not hit and p.get('promotor') and p.get('municipio'):
            hit = prom_mun.get((norm_name(p['promotor'])[:30], norm_name(p['municipio'])[:25]))
        if hit:
            p['set_vinculada'] = hit[0]
            p['set_match'] = 'heredado'
            p['set_fuente'] = 'hermano'
            heredados += 1
    print(f"  heredados de proyectos hermanos: {heredados}")
    n2 = len(pj['proyectos'])
    cv = sum(1 for p in pj['proyectos'] if p.get('set_vinculada'))
    print(f"SET vinculada FINAL: {cv} ({cv*100//n2}%)")
    json.dump(pj, open(out_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
