...
def _nombre_desde_titulo(titulo: str, datos: dict) -> str:
    ...
    titulo_limpio = re.sub(
        r'^(Resolución|Orden|Decreto|Anuncio|Extracto|Notificación)\s+(de\s+\d+\s+de\s+\w+\s+de\s+\d+,?\s+)?',
        '', titulo, flags=re.IGNORECASE)
    titulo_limpio = re.sub(r',.*$', '', titulo_limpio).strip()
    # Rechazar si el fragmento es solo intro administrativa (no es nombre de proyecto)
    _BAD_STARTS = (
        "de la delegación", "del departamento", "por el que", "por la que",
        "de la dirección", "del servicio", "de la subdirección",
        "al haber transcurrido", "en virtud",
    )
    if any(titulo_limpio.lower().startswith(b) for b in _BAD_STARTS):
        titulo_limpio = ""
    palabras = titulo_limpio[:60]
    return palabras if len(palabras) > 10 else None
