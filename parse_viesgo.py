...
Viesgo Distribución (absorbida por UFD/Naturgy desde 2025) desde el Excel
'references/Mapas_Capacidad_AyC-Viesgo.xlsx'.

El Excel contiene histórico desde jul-2021 hasta feb-2025.
No se espera que reciba más actualizaciones (empresa absorbida).

Dos layouts a lo largo del tiempo (detección automática por cabecera):
  Layout A (hasta ~oct-2023): cabecera en fila 3, sin columna CONCATENATE
    col 1: PROVINCIA | col 5: SUBESTACIÓN | col 6: TENSIÓN | col 12: DISPONIBLE
  Layout B (desde ~nov-2023): cabecera en fila 4, con columna CONCATENATE
    col 2: PROVINCIA | col 6: SUBESTACIÓN | col 7: TENSIÓN | col 15: DISPONIBLE

Salida: references/viesgo_capacidad.csv
  fecha, distribuidora, provincia, municipio, subestacion, tension_kv, disponible_neta

Uso:
  python parse_viesgo.py
  python parse_viesgo.py --summary
"""
...
SRC  = BASE / "references" / "Mapas_Capacidad_AyC-Viesgo.xlsx"
OUT  = BASE / "references" / "viesgo_capacidad.csv"

# Incluye typos conocidos ('diembre' en lugar de 'diciembre')
MESES_ES = {
    'enero':1,'febrero':2,'marzo':3,'abril':4,'mayo':5,'junio':6,
    'julio':7,'agosto':8,'septiembre':9,'octubre':10,'noviembre':11,
    'diciembre':12,'diembre':12  # typo en hoja '15 diembre 2023'
}
MAX_MW   = 5000.0
...
def detect_layout(df):
    """
    Detecta la fila de cabecera buscando SUBESTACIÓN y CAPACIDAD DISPONIBLE.
...
for sheet in data_sheets:
    ...
    for _, row in df.iloc[hdr_idx + 1:].iterrows():
        ...
        if disp is None or disp > MAX_MW:
            n_skip += 1
            continue
        records.append([fecha, 'Viesgo', prov, '', set_name, kv, round(disp, 4)])
        n_ok += 1
