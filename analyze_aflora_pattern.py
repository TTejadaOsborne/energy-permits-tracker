...
def main():
    ...
    # Export JS para la herramienta (panel de base empirica)
    sem = Counter((l-1)//6 for l in lags)
    js = {'generado': str(__import__('datetime').date.today()),
          'n': n, 'p25_d': q(0.25)*30, 'mediana_d': q(0.5)*30, 'p75_d': q(0.75)*30,
          'umbral_mw': UMBRAL_MW, 'ratio_min': RATIO_MIN, 'ratio_max': RATIO_MAX,
          'ventana_meses': VENTANA_MESES,
          'semestres': {'S1': sem.get(0,0), 'S2': sem.get(1,0), 'S3': sem.get(2,0)},
          'casos': sorted(casos, key=lambda c: c['lag_meses'])}
    (BASE/'aflora_cases.js').write_text('window.AFLORA_CASES = ' + json.dumps(js, ensure_ascii=False) + ';', encoding='utf-8')
    print(f'\nCSV: {out}')
    print('JS : aflora_cases.js')
