...
def run(fecha=None, boletines=None, max_items=None, api_key=None, modelo="haiku", solo_scraping=False):
    ...
    if not items:
        ...
        # Crear JSON vacío para marcar la fecha como procesada
        import pathlib as _pl, json as _jj
        _odir = _pl.Path("output"); _odir.mkdir(exist_ok=True)
        _of = _odir / f"energy_extraido_{fecha}.json"
        if not _of.exists():
            _jj.dump({"fecha": fecha, "total": 0, "energeticos": 0,
                      ...
                      "resultados": []}, _of.open("w", encoding="utf-8"))
        return None
