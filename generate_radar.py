...
combinando 4 pilares de anticipacion de afloramientos.
  ...
  P3 ZOMBIS     MW de proyectos en tramitacion sin ninguna publicacion nueva
...
def main():
    ...
    def reg(k):
        return R.setdefault(k, {'mw_ventana':0.0,'mw_ventana_18m':0.0,'tram_drop_3m':0.0,
                                'mw_zombi':0.0,'mw_hitos_6m':0.0,
                                'n_fallidos':0,'n_zombis':0,'n_hitos':0,
                                'fallidos':[],'zombis':[],'hitos':[]})

    # ── P1 ventana + P3 zombis + P4 hitos ──
    for p in pj['proyectos']:
        ...
        if (p.get('estado_actual') or '') in EN_CURSO:
            ...
            if fu and (HOY - fu).days > 730 and mw:
                r = reg(key); r['mw_zombi'] += mw; r['n_zombis'] += 1
                r['zombis'].append({'p': nombre[:48], 'mw': round(mw,1), 'meses_sin_pub': (HOY-fu).days//30})
    ...
    for k, r in R.items():
        ...
        out.append({'set': k, 'score': round(s*100),
                    ...
                    'tram_drop_3m': r['tram_drop_3m'], 'mw_zombi': round(r['mw_zombi']),
                    'mw_hitos_6m': round(r['mw_hitos_6m']),
                    'n_fallidos': r['n_fallidos'], 'n_zombis': r['n_zombis'], 'n_hitos': r['n_hitos'],
                    'fallidos': r['fallidos'][:5], 'zombis': sorted(r['zombis'], key=lambda z:-z['mw'])[:5],
                    'hitos': r['hitos'][:5]})
    out.sort(key=lambda x: -x['score'])
    out = out[:200]

    data = {'generado': HOY.isoformat(),
            'pesos': {'ventana': 0.35, 'tramite': 0.30, 'zombis': 0.20, 'hitos': 0.15},
            'sets': out}
    Path(BASE/'radar_data.js').write_text('window.RADAR_DATA = ' + json.dumps(data, ensure_ascii=False) + ';',
                                          encoding='utf-8')
    print(f'SETs en radar: {len(out)}')
    for r in out[:10]:
        print(f"  {r['score']:3d}  {r['set'][:26]:26} vent={r['mw_ventana']:5d}  tram3m={r['tram_drop_3m']:7.1f}  zombi={r['mw_zombi']:5d}  hitos={r['mw_hitos_6m']:5d}")
